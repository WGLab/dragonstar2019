This is a tutorial for how to use PLINK for data analysis. Please note that the data used here is simulation data provided by the software developers.
...
```
where the first part is similar to the output of the previous command, but more log information is for hapmap1.bed, hapmap1.bim and hapmap1.fam which generated by this command. 

Alternatively, you can also generated bed file with high genotyping using the command below:
```
...
```
The first command here will save output to `as1.assoc`, and the second command will get the top 10 association statistics as what is shown below.
```
